...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_question'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO secondary_questions (label, type, options, position) VALUES (?, ?, ?, ?)');
    ...
    if (isset($_POST['edit_question'])) {
        ...
        $options = ($type === 'multi_choice') ? cleanInput($_POST['options']) : null;
        $position = intval($_POST['position']);
        $stmt = $pdo->prepare('UPDATE secondary_questions SET label=?, type=?, options=?, position=? WHERE id=?');
    ...
    if (isset($_POST['move_up']) || isset($_POST['move_down'])) {
        ...
        $stmt = $pdo->prepare('SELECT position FROM secondary_questions WHERE id=?');
        $stmt->execute([$id]);
        $current = $stmt->fetchColumn();
        $new = $current + $direction;
        $stmt = $pdo->prepare('SELECT id FROM secondary_questions WHERE position=?');
        $stmt->execute([$new]);
        $swap_id = $stmt->fetchColumn();
        if ($swap_id) {
            $pdo->prepare('UPDATE secondary_questions SET position=? WHERE id=?')->execute([$new, $id]);
            $pdo->prepare('UPDATE secondary_questions SET position=? WHERE id=?')->execute([$current, $swap_id]);
        }
    }
}
...
$smarty->display($theme_current . '/admin/manage_secondary.tpl');
